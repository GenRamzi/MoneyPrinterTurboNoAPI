...
payload='{"topic":"Benchmark","script":"This benchmark measures the complete local rendering path.","duration":10,"provider":"gemini","voice":"en-US-AvaNeural","subtitles":true,"subtitle_format":"ass","gpu_backend":"auto","batch_count":1,"aspect_ratio":"16:9","clip_duration":2}'
...
for _ in $(seq 1 120); do
  ...
  if [ "$state" = "completed" ]; then
    ...
    printf 'backend=%s encoder="%s" elapsed_seconds=%.3f task=%s\n' "$backend" "$encoder" "$(awk "BEGIN {print $elapsed_ms / 1000}")" "$task_id"
    exit 0
  fi
